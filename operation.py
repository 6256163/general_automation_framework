...
class Operation(object):
    ...
    def verify(self, value, property, *loc):
        ...
        elif property.upper() == "VALUE":
            actual_result = self.get_element(*loc).get_attribute(property)
        else:
            raise Exception

        if actual_result.encode("utf-8") == value:
    ...
    ####################### For Public Function################
    def get_element(self, *loc):  # loc <tuple>: (by,value)
        try:
            # 注意：以下入参为元组的元素，需要加*。Python存在这种特性，就是将入参放在元组里。
            # WebDriverWait(self.driver,10).until(lambda driver: driver.find_element(*loc).is_displayed())
            # 注意：以下入参本身是元组，不需要加*
            WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located(*loc))
            return self.driver.find_element(**dict(zip(("by", "value"), *loc)))
